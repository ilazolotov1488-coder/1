        ...
        if (expandedAnimation.getValue() != 0) {
            ...
            for (ModeSetting.Value mode : modes) {
                ...
                if (mode == setting.getValue()) {
                    ctx.drawRoundedRect(dropdownX + 1f, optionY, dropdownWidth - 2f, 13,
                            mode == modes.getLast() ? BorderRadius.bottom(3, 3) : BorderRadius.all(0),
                            enabledColor.mulAlpha(expandedAnimation.getValue()));
                    ...
                    ctx.drawText(optionFont, mode.getName(), dropdownX + 6, optionY + (13 - optionFont.height()) / 2f,
                            disableColor.mulAlpha(expandedAnimation.getValue()));
                }
